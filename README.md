...
| 面板 | 是否支持 | 说明 |
|------|:--------:|------|
| [Xboard](https://github.com/cedar2025/Xboard) | ✅ | 完全支持，原生插件架构 |
| V2board | ❌ | 不支持（无插件系统） |
| SSPanel-UIM | ❌ | 不支持（架构不同） |
| NeXboard | ⚠️ | 如兼容 Xboard 插件规范则可用 |

> 💡 本插件基于 Xboard 原生插件系统开发，仅支持具有 `plugins/` 插件架构的 Xboard 面板。
